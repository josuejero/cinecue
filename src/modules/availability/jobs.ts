...
import {
  syncFutureReleasesCatalog,
  syncShowingsByZip,
  syncTheatresByZip,
} from "@/modules/availability/ingestion/sync";
import { NotFoundError } from "@/shared/http/errors";
import {
  refreshLocationReadModel,
  refreshMovieLocalStatusForLocation,
} from "@/modules/availability/read-model";
import { isGracenoteRequestNotAuthorizedError } from "@/integrations/gracenote/client";
import { getLocationCluster, listActiveLocationClusters } from "@/modules/availability/location-clusters";
...
} from "@/modules/ops/worker-runtime";
import {
  enqueueLocationSync,
...
export async function enqueueActiveLocationSyncs(limit?: number) {
  ...
  const clusters = await listActiveLocationClusters(limit ?? env.AVAILABILITY_ACTIVE_LOCATION_LIMIT);

  for (const cluster of clusters) {
    await enqueueLocationSync({
      locationId: cluster.locationId,
      reason: "scheduled-fanout",
      numDays: env.AVAILABILITY_SYNC_NUM_DAYS,
    });
  }
...
export async function syncLocationCluster(input: {
  ...
  const numDays = input.numDays ?? env.AVAILABILITY_SYNC_NUM_DAYS;
  ...
  await touchLocationSyncState({
    ...
    staleAfterSeconds: env.AVAILABILITY_STALE_AFTER_MINUTES * 60,
  });

  await enqueueEmailNotificationProcessing({
    locationId: cluster.locationId,
    reason: `after-location-sync:${cluster.locationId}`,
    limit: env.NOTIFICATIONS_EMAIL_BATCH_SIZE,
  });

  await enqueuePushDeliveryProcessing({
    locationId: cluster.locationId,
    reason: `after-location-sync:${cluster.locationId}`,
  ...
  writeLog("info", "availability.location_sync.completed", {
    locationId: cluster.locationId,
...
export async function syncFutureReleaseCatalog(input?: {
  ...
  try {
    const summary = await syncFutureReleasesCatalog({
      releaseDate,
      numDays: input?.numDays ?? env.CATALOG_FUTURE_RELEASES_NUM_DAYS,
      country: input?.country ?? env.AVAILABILITY_SYNC_COUNTRY,
    });

    return summary;
  } catch (error) {
    if (isGracenoteRequestNotAuthorizedError(error)) {
      writeErrorLog("catalog.future_releases.skipped", error, {
        releaseDate,
      });
...
export async function runRuntimeCleanup(input?: {
  ...
  const [showtimesSummary, jobRunsSummary] = await Promise.all([
    cleanupOldShowtimeRows(input?.showtimeRetentionDays ?? env.AVAILABILITY_SHOWTIME_RETENTION_DAYS),
    cleanupOldWorkerJobRuns(input?.jobRunRetentionDays ?? env.WORKER_JOB_RUN_RETENTION_DAYS),
  ]);
