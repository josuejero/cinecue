...
function userTagKey(userId: string) {
  return `phase6:dashboard-cache:user:${userId}`;
}
...
export async function writeDashboardCache<T>(
  ...
  await redis
    .multi()
    .set(key, JSON.stringify(payload), "EX", env.PHASE6_DASHBOARD_CACHE_TTL_SECONDS)
    .sadd(tagKey, key)
    .expire(tagKey, env.PHASE6_DASHBOARD_CACHE_TTL_SECONDS)
    .exec();
}
